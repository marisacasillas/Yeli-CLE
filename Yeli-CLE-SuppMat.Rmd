...
As more data are added to this corpus, the analyses will also be updated, as will this supplementary model information, all of which will be available online at: https://middycasillas.shinyapps.io/Rossel_Child_Language_Environment/.
...
apa_table(filter(model.results, model == "ODS_random_nb.v2") %>% select(-model),
          caption="Model output of the negative binomial mixed-effects regression of ODS min/hr for the random sample, with afternoon as the reference level for time of day.")
```
