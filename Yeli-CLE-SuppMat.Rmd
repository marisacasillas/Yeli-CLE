...
As more data are added to this corpus, the analyses will also be updated, as will this supplementary model information, all of which will be available online at <OMITTED FOR REVIEW><!--: https://middycasillas.shinyapps.io/Yeli_Child_Language_Environment/-->.
...
apa_table(filter(model.results, model == "TCDS_random_z-inb.v2") %>% select(-model),
          caption="Full output of the zero-inflated negative binomial mixed-effects regression of TCDS min/hr for the random sample, with afternoon as the reference level for time of day.")
```
...
apa_table(filter(model.results, model == "TCDS_random_log_gaus.v2") %>% select(-model),
          caption="Full output of the gaussian mixed-effects regression of TCDS min/hr for the random sample, with afternoon as the reference level for time of day.")
```
...
``` {r fig4, echo=FALSE, fig.align = "center", fig.cap="The distribution of TCDS rates found across the 55 turn-taking clips.", out.width = '40%'}
...
apa_table(filter(model.results, model == "TCDS_turntaking_nb.v2") %>% select(-model),
          caption="Full output of the negative binomial mixed-effects regression of TCDS min/hr for the turn-taking sample, with afternoon as the reference level for time of day.")
```
...
apa_table(filter(model.results, model == "TCDS_turntaking_log_gaus.v2") %>% select(-model),
          caption="Full output of the gaussian mixed-effects regression of TCDS min/hr for the turn-taking sample, with afternoon as the reference level for time of day.")
```
...
apa_table(filter(model.results, model == "ODS_random_nb") %>% select(-model),
          caption="Full output of the negative binomial mixed-effects regression of ODS min/hr for the random sample, with midday as the reference level for time of day.")
```

```{r tab10, echo=FALSE, results='asis', message=FALSE, warning=FALSE}
apa_table(filter(model.results, model == "ODS_random_nb.v2") %>% select(-model),
          caption="Full output of the negative binomial mixed-effects regression of ODS min/hr for the random sample, with afternoon as the reference level for time of day.")
```
...
knitr::include_graphics("www/ODS_random_nb_res_plot.png")
```
...
apa_table(filter(model.results, model == "ODS_random_log_gaus.v2") %>% select(-model),
          caption="Full output of the gaussian mixed-effects regression of ODS min/hr for the random sample, with afternoon as the reference level for time of day.")
```
...
``` {r fig10, echo=FALSE, fig.align = "center", fig.cap="The distribution of ODS rates found across the 55 turn-taking clips.", out.width = '40%'}
...
apa_table(filter(model.results, model == "ODS_turntaking_nb") %>% select(-model),
...
apa_table(filter(model.results, model == "ODS_turntaking_nb.v2") %>% select(-model),
          caption="Full output of the negative binomial mixed-effects regression of ODS min/hr for the turn-taking sample, with afternoon as the reference level for time of day.")
```

\FloatBarrier

``` {r fig11, echo=FALSE, fig.align = "center", fig.cap="The model residuals from the negative binomial mixed-effects regression of ODS min/hr for the turn-taking sample.", out.width = '90%'}
knitr::include_graphics("www/ODS_turntaking_nb_res_plot.png")
```
...
apa_table(filter(model.results, model == "ODS_turntaking_log_gaus.v2") %>% select(-model),
          caption="Full output of the gaussian mixed-effects regression of ODS min/hr for the turn-taking sample, with afternoon as the reference level for time of day.")
```
