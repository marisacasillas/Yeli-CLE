...
The Panoramic approach has many significant drawbacks (REFS), particularly for research questions that involve linguistic analysis. Here we focus on those drawbacks that prevail even when we assume that the researcher has the resources to add manual or automated annotation of some linguistic phenomena. First, the resulting data collections are typically too large for comprehensive transcription and annotation, with no easy way to scan for the specific linguistic or interactional phenomena of theoretical interest. Researchers must then employ some strategic sub-sampling techniques before annotating the data, though best practices are still not well established (REFS). Second, even once clips are sub-sampled, adding relevant annotations to them can take nearly as long as a Close-Study approach with select videos, but with an additional reduced likelihood of capturing behaviors that are considered relevant for caregiving and language development in the community. A third drawback is that properly collecting, processing, and archiving daylong data is not easily achieved; the fact that participants are likely to habituate to the recorder is terrific for documenting ecologically valid language use, but raises urgent questions about participant privacy standards (REFS Ethics). Fourth, at the time of writing, there are few options for capturing visual information across the day (but see REFS), limiting this method primarily to acoustic phenomena (REFS). Even if researchers add manual annotation to these audio files, they typically do so without the benefit of the visual context; a difficultly compounded by the diversity of activities and interlocutors captured over the recording.

## Trade-offs of the close and panoramic views
