...
# Introduction {#intro}
In their first few years of life, children hear an extraordinary amount of language. The sum of this experience with language (their "input")  is the basis for their lexical, grammatical, and sociolinguistic development. Much developmental language research focuses on the value of child-directed speech as a tailored source of linguistic input that can boost lexical and syntactic development [@bates1997inseparability; @brinchmann2019direct; @frankIPvariability; @hart1995meaningful; @hoff2003specificity; @huttenlocher2010sources; @lieven1997lexically; @marchman2004language; @shneidman2012language; @weisleder2013talking]. However, we also know that language environments---e.g., who is around, talking about what to whom---vary dramatically within and across families, **and** children in some communities **hear** very little directed talk **without any** apparent delays in their linguistic development [@brown2011cultural; @brown2014interactional; @brown2014language; @casillas2019early; @gaskins2006cultural; @ochs1984language]. **A key puzzle for developmental language science** is then unmasking how the human cognitive toolkit for language learning can flexibly adapt to the variable contexts under which it successfully occurs. A first step along the way **to resolving this puzzle is documenting** this variation. **But tracking the distribution and characteristics of linguistic input over multiple interactional contexts, across developmental time, between different families, and across different cultural groups is an exceedingly difficult task.**

## Tracking linguistic input in diverse communities

Traditionally, **developmentalists have** relied on short cross-sectional or longitudinal video recordings of caregiver-child interaction, at home or in the lab, to get a grasp on what kinds of language children typically hear. This approach has been fruitful in teasing out individual and group-based differences in interactional behaviors [@cartmill2013quality; @hoff2003specificity; @hurtado2008does; @rowe2008child]**, yielding rich insights into the moment-to-moment characteristics of interaction in both over- and understudied populations [REFS]. The short nature of these recordings forces researchers to thougtfully consider how to sample interactional contexts; these considerations include the perceived typical childrearing attitudes and practices, as assessed by the researcher, via their community membership or ethnographic study of the community [REFS]. Even when done thoughtfully, however, this method is limited in its ability to capture the wide variety of activities involving language encountered by young children. For example, the anchoring effect of a single video camera on the child (and caregivers) makes it difficult to capture activities that involve a lot of free motion (e.g., talking while running around) or activities that are not readily accessible to others (e.g., pre-sleep routines).**

Daylong recordings**, an alternative to the traditional short video method,** are typically made from a single audio recorder worn by the target child at home, unleashing participants from the constraint of being within direct view of a fixed camera or a mobile camera operator, and thereby allowing them to more freely navigate their environment for multiple hours at a time **[@casillas2019stepbystep]. While daylong recording corpora quickly grow into thousands of hours of data---often featuring only patchy vocal activity---advances in speech technology increasingly allow researchers to use automated methods to reliably identify target child and female adult speech robustly across a growing number of languages and settings [REFS, ACLEW; @xu2009reliability]. Additionally, well-documented methods for systematically adding manual annotations to sub-parts of these recordings are expanding rapidly, some of which builds directly on past work with short recordings [REFS;@cychoszURcanonical, @casillas2019stepbystep, @casillas2017DAS]. In brief, daylong recordings finally make it easy for researchers to document both the ebbs _and_ the flows in everyday language activity (e.g., to see how linguistic input varies across speakers, activities, etc.). Because they are used for whole waking days, this recordings release reseachers from choosing individual interactional contexts, and can therefore be collected using similar procedures across diverse linguistic and cultural settings [@bergelsonIPbsl; @cychoszURcanonical].**

**Indeed, collecting daylong recordings and analyzing them with automated tools is so easy that this new method presents a hidden risk when the research team is not sufficiently familiar with the community under study: basic patterns in language exposure, when viewed exclusively through the researcher's (alien) cultural perspective, risk mischaracterizing how language is used and learned in that community (see also @cychoszURlongform). Short of the ideal scenario---more researchers from more diverse communities in developmental science---a first critical aim is to investigate which aspects of cross-cultural variation in the child language environment are captured in daylong recordings. For example, 

STUFF MIGHT BE SHAPED BY CAREGIVER IDEOLOGY. OR FOR EXAMPLE MAYBE BY LIFESTYLE.

how closely basic input measures  reflect cross-cultural variation in child language environments and child language development.

approaches to child development, social and economic organization, and more. In the current paper, we present the second part of a comparative study that brings together findings from both careful ethnographic work and daylong recordings in two unrelated communities. We ask whether daylong language input patterns are more reflective of the simliar lifestyles between these two communities (subsistence farming, multiple caregivers, multi-generational patrilocal housing) or their divergent perspectives on how and how often to talk to children.**

<!--focus on diffs between communities (downplay US data since they're not directly comparable; explain in response to revs)-->
